...
def launch_scenarios(Wout):
	...
	for i_episode in range(1):
		...
		for t in range(100):
            ...
            features=np.random.random(1025)
			features[-1]=1
            print(Wout.shape,features.shape)
            actions=np.clip(Wout@features,-1,1)
...
mu=np.random.random(1024)
C=np.identity(1024)

def progtot():
	for iterate in range(2):
		mu=np.random.random(1025*3)
		C=np.identity(1025*3)
		sig=0.5
		rng = np.random.default_rng()
		Wout=rng.multivariate_normal(mu,C*sig)
		launch_scenarios(np.reshape(Wout,(3,int(Wout.size/3)))
		print(Wout)
"""
