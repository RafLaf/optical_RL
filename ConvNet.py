...
def testnet(nparray,rho,aleak):
    #nparray= np.random.random((nbofimages=1,RVB=3,pixelh=96,pixelv=96))
    net = Net(rho)
    A = torch.from_numpy(nparray)
    return net.forward(A.float()),net.RCstep(A.float(),aleak)
...
class Net(nn.Module):
    ...
    def RCstep(self,x,aleak):
        ...
        return torch.cat((self.r,a))   #concatenation of reservoir and instananeaous states
