...
def progtot():
    ...
    while not es.stop():
        ...
        print("mu",mu)
        time.sleep(10)
        es.tell(Wout, [utils.game.launch_scenarios(Wouti/500) for Wouti in Wout])
        es.disp()
        np.save('mu.npy',mu)
        np.save('C.npy',C)
        print("\n \n NEW ITERATION \n \n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        W=W.A
        print(max(abs(np.linalg.eigvals(W))))
        np.save('W.npy',W)
    ...
    try :
        ...
        torch.save(net.state_dict(), 'modelstate.pt')
        net.eval()
        print('loaded net')
    except FileNotFoundError:
        print('creating net')
        net=utils.network.initnet(dtype,W)
